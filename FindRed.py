...
class GoPiGoBot():
    ...
    def move(self,dist,how="F"):
        ...
        if(dist<30 and (how=="F" or how=="B")):
            ...
            return 1
    def searchPattern(self,rotdist=3):
        samples = []
        #self.move(rotdist*2,"TR")
        avg = getAverageColor(self.camera,self.stream)
        samples+= [float(avg[2])/sum(avg)] #this is red's relative contrubtion
        
        redness = float(avg[2])/sum(avg) #this is red's relative contrubtion

        if(random.Random()<redness):
                self.move(rotdist,"TL")
        else:
                self.move(rotdist,"TR")
        #for a in range(5):
        #    self.move(rotdist,"TL")
        #    avg = getAverageColor(self.camera,self.stream)
        #    samples+=[float(avg[2])/sum(avg)] #this is red's relative contribution

        #bestrotation = samples.index(max(samples))
        #for a in range(bestrotation):
        #self.move(rotdist*(len(samples)-bestrotation-1),"TR") #since we rotated left initially, now
                                          #we go right
        #[samples[bestrotation]*10]+[bestrotation]
        return [redness*100]
    def exploit(self):
        #exploit by moving back and forth. Oh yeah!
        self.move(40,"F")
        self.move(40,"R")
...
if(__name__=="__main__"):
    ...
    with picamera.PiCamera() as camera:
        ...
        for x in range(100):
            ...
            if(random.random()<float(average[2])/sum(average)):
